...
必須は、どのレコードにも必ず入るかどうかである。✓ でない属性が入るかはログ種別ごとに決まり、その対応は各コンポーネントのイベント一覧が持つ。

同じ名前の属性は、コンポーネントをまたいでも同じ意味で使う。
...
Lambda 関数内の初期化処理で失敗した場合は、任意のエラーメッセージが `msg` に入る。
...
エラーが発生した場合、以下においてエラーの発生が記録もしくは通知される。

- ログ
- `status#` の `last_error`
- Amazon SNS への通知

ただし次のエラーは、ログにのみ記録される。これは、記録の失敗を表現するエラーであるためである。
...
`action` が `doctor-prune` の場合は `group` が含まれない。`pruned` が付くのは、prune 自体は動いたが個々のレコードの削除に一部失敗した場合のみである。

起動できなかった場合は、上記のイベント外で ERROR を 1 行出し、終了コード 1 で終了する。
