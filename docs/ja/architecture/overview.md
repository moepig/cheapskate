...
アクションの直前には一意な `operation_id` を含む pending operation を Status に条件付きで記録し、状態変更 API コールの成功後に完了へ更新する。完了更新の前に実行が途切れた場合、次のサイクルは観測状態から結果を確定し、同じアクションを再送しない。通知は同じ処理内で最大 2 回送信し、ともに失敗した場合は送信を打ち切る。通知の失敗は後続する AWS 操作を停止しない。
...
reconciler は、同じ理由により、reconcile 中の一致状況を根拠として `status#` を即時削除しない。代わりに、Status の更新ごとに期限を延長し、一定期間更新されなかったアイテムを DynamoDB TTL が削除する。この方式は Tagging API の一時的な遅れを孤立判定に使わない。
