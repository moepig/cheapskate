...
アクションの直前には一意な `operation_id` を含む pending operation を Status に条件付きで記録し、成功後に完了へ更新する。完了更新の前に実行が途切れた場合、次のサイクルは観測状態から結果を確定し、同じアクションを再送しない。通知も `operation_id` と送信待ち状態を Status に保持して再試行する。
