...
名前空間は `METRICS_NAMESPACE`(既定 `cheapskate`)、次元は持たず、単位はすべて Count である。
...
グループ名を次元にすると、カスタムメトリクスの数が利用者のグループ数に比例して増える。cheapskate は本体のコストを月 1 ドル未満に収めることを目標としており、グループ数に比例してメトリクス料金が本体を上回る構成を取らない。
...
`METRICS_ENABLED` が解釈できない値のときは、既定へ倒さず起動を失敗させる。打ち間違いを黙って有効と読むと、切ったつもりの課金が続き、気づく手段が請求書しかなくなるためである。無効時はコールドスタートごとに `metrics-disabled` のログを 1 行出力する。

有効・無効の判定は `cloudwatch.Emitter` 型自身が持ち、呼び出し側に条件分岐を書かせない。発行箇所が複数あり、そのすべてに同じ条件を書かせると片方だけ書き忘れる余地が残るためである。

無効時に失われるのは件数と推移だけである。
...
EMF で生成されるメトリクスはカスタムメトリクスであり、CloudWatch の課金対象となる。
...
メトリクスを有効にすると、それだけで本体(Lambda + DynamoDB オンデマンド)の月額を上回りうる。本体のコストが月 1 ドル未満であるという記述はコンピュートとストレージについてのものであり、可観測性の費用を含まない。
