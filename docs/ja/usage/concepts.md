...
cheapskate は、AWS リソースのあるべき状態を登録し、実際の状態をそれに合わせ続けるツールである。本ドキュメントは、全体の流れを示し、usage の各ドキュメントで用いる用語を定義する。

## 全体の流れ

設定の投入から実状態の変更までの流れを表すと、次の図のようになる。
...
設定変更とタグの付け外しは、いずれも次の reconcile サイクルから反映される。望ましい状態と実状態が一致している場合、リソースへの操作は行われない。

## 用語

本ドキュメント以降で用いる用語を、以下に示す。
