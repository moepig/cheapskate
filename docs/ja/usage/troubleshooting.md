...
reconciler は全体の読み込みを完了したサイクルの最後に `summary` レコードを出力する。全体の読み込みに失敗した場合は summary を出力せず、Lambda のエラーとして返す。Describe まで到達したリソース数、成功したアクション数、およびグループまたはリソースのエラー数を確認する。カスタムメトリクスを有効にしている場合、`ReconcileErrors` と `ReconcileAborted` を alarm の入力にできる。対象グループ、ARN、および原因はログで確認する。

設定 Query または全体のリソース検出に失敗した場合、リソースの Describe と変更を始める前にサイクルを中断する。DynamoDB または `tag:GetResources` の権限と到達性を確認する。探索中の ARN 解析エラーも全体中断になるため、エラーに示された ARN の形式も確認する。原因を修正してから再実行する。
...
| 失効後に schedule を保存できない | 失効済み override を clear-override で削除するか、新しい override で置き換えてから schedule を保存する |
...
RDS と EC2 で遷移中と判定したリソースは意図的に処理を見送る。ECS のタスク数の不一致は停止操作を抑止しない。後続の 5 分サイクルで再度観測する。Start と Stop の配送は at-least-once であるため、adapter と運用手順は絶対状態の要求を繰り返しても処理できる必要がある。
