...
The `-output text|json` option selects output. JSON `list` always emits one object with `groups` and `errors`. Text `list` sends valid groups to stdout and invalid groups to stderr.
