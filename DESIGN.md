...
3. **設定の CRUD を行う CLI ツール(`cheapskate-cli`)を提供する**。インタフェースは CRUD の形にとらわれず、実態の操作としては DynamoDB のアイテムを扱う
...
```
  ...
  rate(N minutes) ────┐   │  state テーブル           │◄── cheapskate-cli (CLI) / Terraform /
...
| `cheapskate-cli` | ソース(`go build ./cmd/cheapskate-cli`)/ リリースバイナリ | 設定アイテムの操作 |
...
│   ├── ops/                 # 設定操作の共通ロジック(cheapskate-cli と webconsole で共用)
...
```
pk = "config#<resource_id>"     ← ユーザー(cheapskate-cli / Terraform)が書く。Reconciler は読むだけ
----------------------------------------------------------------
...
pk = "override#<resource_id>"   ← 人が一時的に書く(cheapskate-cli override)。TTL 属性で自動消滅
----------------------------------------------------------------
...
- **status# の各フィールドは last_action_at / last_error_at 時点のスナップショットであり、現在のライブ状態ではない(B-10)**。収束サイクル(何もアクションしない周期)では書き込みを行わない設計(全リソースを 5 分ごとに毎回書き込むコストを避けるため)なので、cheapskate の外側でリソースが変更されると `observed_state` は古いまま残り得る。cheapskate-cli list / show のヘッダは `OBSERVED(AT LAST ACTION)` と明記し、webconsole も同様に表示する

- `resource_id` 例: `rds-cluster#my-aurora`、`rds-instance#my-db`、`ecs#my-cluster/my-service`。型プレフィックスから `type` が導出できるため、CLI では型指定が不要
- desired の決定順序: **mode=disabled は override より強く常にスキップ**。disabled でなければ **override(期限内)> mode=pinned の desired > cron 評価**。`cheapskate-cli override` / webconsole は disabled な config への override 登録を拒否する(スケジュール/pin してから override すること)
...
```
cheapskate-cli list                                        # 登録リソース一覧(override/status を結合表示)
cheapskate-cli show <resource-id>                          # config + override + status を JSON で
cheapskate-cli pin <resource-id> running|stopped           # 固定(旧 cron 設定は保持され schedule で復帰可)
cheapskate-cli schedule <resource-id> -start CRON -stop CRON [-timezone TZ] [-restore-count N]
cheapskate-cli disable <resource-id>                       # 設定を残したまま管理停止
cheapskate-cli override <resource-id> running|stopped -for 2h   # 期限付き上書き(TTL で自動消滅)
cheapskate-cli override <resource-id> -clear
cheapskate-cli remove <resource-id>                        # config/override/status を削除
```
...
- config アイテムを Terraform 管理にしている場合、`cheapskate-cli pin/schedule/disable/remove` の書き込みは Terraform とドリフトする。恒久設定は IaC・一時操作は `cheapskate-cli override`、という使い分けを README で案内する(override# は TTL 付きで IaC 管理しない前提)
- store 層は Reconciler と共用。CLI 固有のデータアクセスコードを持たない
- 操作ロジック(検証 + store 呼び出し)は `internal/ops` に置き、Web コンソールと共用する

## Web コンソール(オプトイン)

必要な場合だけユーザーが API Gateway + Lambda を追加構築するオプトインのブラウザフロントエンド。cheapskate-cli と同じ操作を `internal/ops` 経由で提供する。
...
- 責務分離は cheapskate-cli と同じ: 実行ロールは state テーブルの Scan/Get/Put/Delete のみで、RDS/ECS API は呼ばない
...
- **IaC なし**: setup.md の AWS CLI 実行例をそのまま実行、登録は `cheapskate-cli`
...
| 結合(go test -tags integration) | store の実 DynamoDB 呼び出し、リコンサイル一連動作(Scan → Describe → Stop/Start → status 書込 → SNS 実配信)、cheapskate-cli の各コマンド | Floci(`make floci-up`)。SNS 配信は SQS 購読プローブで実受信を検証 |
...
| 設定インタフェース | `cheapskate-cli` CLI(意図ベースの動詞、実態は DynamoDB アイテム操作) | 要件。DynamoDB 以外の AWS API は呼ばず責務分離を維持 |
| IaC 向け成果物 | ドキュメント内の AWS CLI 実行例のみ(テンプレート/モジュール/Construct は配布しない) | 既存のコーディング規則に別規則を持ち込まないため |
| 恒久設定と一時操作 | 恒久 = IaC または cheapskate-cli、一時 = cheapskate-cli override(TTL) | Terraform 管理 config とのドリフトを役割分担で回避 |
